...
from hackathon.utils import (send_activation_email,
...
class SignUpView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        request = self.request
        user = self.extract_user_from_form(form)
        ...
        self.validate_login_form(form, request, user)

        return redirect('index')

    def extract_user_from_form(self, form):
    ...
    def validate_login_form(self, form, request, user):
        if settings.ENABLE_USER_ACTIVATION:
            self.validate_activation(request, user)
        else:
    ...
    def validate_activation(self, request, user):
